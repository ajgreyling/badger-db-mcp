...
This guide will walk you through setting up Badger DB MCP and connecting it to an AI tool for the first time. We'll use demo mode to get started quickly, then show you how to connect to your own database.
...
Badger DB MCP supports two [transport protocols](/config/command-line#transport):

| Transport | How It Works | Best For |
|-----------|--------------|----------|
| **stdio** | AI tool spawns Badger DB MCP as a subprocess | Local desktop apps, single-user setups |
| **http** | Badger DB MCP runs as a standalone HTTP server | Web clients, shared servers, remote access |

## Step 1: Start Badger DB MCP in Demo Mode

<Note>
If using **stdio** transport, skip this step - your AI tool will spawn Badger DB MCP as a subprocess automatically.
</Note>

For **http** transport, start Badger DB MCP server manually:
...
<Tabs>
  ...
  <Tab title="Docker">
    ```bash
    docker run --rm -p 8080:8080 badger-db-mcp --transport http --port 8080 --demo
    ```
  </Tab>
</Tabs>
...
Badger DB MCP provides an [agent skill](https://github.com/ajgreyling/badger-db-mcp/tree/main/skills/badger-db-mcp) that teaches your AI tool the best-practice workflow for database querying — explore the schema first, then write precise SQL. This reduces errors and back-and-forth.

```bash
npx skills add ajgreyling/badger-db-mcp
```
...
Once you've verified Badger DB MCP works in demo mode, connect it to your actual database.
