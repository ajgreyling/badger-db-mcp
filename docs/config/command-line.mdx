...
Badger DB MCP supports three configuration methods with the following priority order:
...
Command-line flags are passed when starting Badger DB MCP. These have the highest priority and override all other configuration methods.
...
<ParamField path="--transport" type="string" env="TRANSPORT" default="stdio">
  ...
  **Options:**
  ...
  npx badger-db-mcp --transport stdio --dsn "..."

  # http - for web clients, workbench, and remote access
  npx badger-db-mcp --transport http --port 8080 --dsn "..."
  ```
</ParamField>
...
<ParamField path="--port" type="number" env="PORT" default="8080">
  ...
  ```bash
  npx badger-db-mcp --transport http --port 3000 --dsn "..."
  ```
</ParamField>
...
<ParamField path="--id" type="string" env="ID">
  Instance identifier to suffix tool names. Useful when running multiple Badger DB MCP instances (e.g., in Cursor).

  Tools will be named `execute_sql_{id}` for each instance.

  ```bash
  npx badger-db-mcp --id prod --dsn "postgres://user:pass@prod-host:5432/db"
  npx badger-db-mcp --id staging --dsn "postgres://user:pass@staging-host:5432/db"
  ```
...
<ParamField path="--demo" type="boolean" default="false">
  Run Badger DB MCP with a bundled SQLite sample "employee" database for testing.

  ```bash
  npx badger-db-mcp --demo --transport http --port 8080
  ```
</ParamField>
...
<ParamField path="--config" type="string" default="./dbhub.toml">
  ...
  ```bash
  npx badger-db-mcp --config ./dbhub.toml --transport http --port 8080
  ```
...
npx badger-db-mcp --dsn "..." --ssh-host mybastion
```

<Note>
- When `--ssh-host` is a plain alias (no dots, not an IP address), Badger DB MCP automatically resolves it from `~/.ssh/config`, reading the `Hostname`, `User`, `IdentityFile`, and `ProxyJump` directives. Explicit flags always override values from the config file.
