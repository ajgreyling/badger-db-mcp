...
TOML configuration is the recommended way to configure Badger DB MCP for multi-database setups and advanced configurations. It provides more flexibility than command-line options or environment variables.

## Overview

TOML configuration enables:
- **Multi-database support**: Connect to multiple databases from a single Badger DB MCP instance
...
When using TOML configuration, Badger DB MCP automatically watches `dbhub.toml` for changes and reloads database connections without requiring a server restart.

**How it works:**

1. Badger DB MCP detects file changes and waits 500ms (debounce) to handle editors that write in multiple steps
...
4. If reconnection fails, Badger DB MCP automatically rolls back to the previous working configuration

```
# Edit dbhub.toml while Badger DB MCP is running — changes apply automatically
...
HTTP transport (`--transport http`) creates a fresh Badger DB MCP server per request, so all changes — including added/removed tools — take effect immediately.
</Note>
...
```bash
# Set env vars, then start Badger DB MCP
...
npx badger-db-mcp --config dbhub.toml
```
...
npx badger-db-mcp --transport http --port 8080

# Or specify custom location
npx badger-db-mcp --config=/path/to/config.toml
```

## Source Options

Sources define database connections. Each source represents a database that Badger DB MCP can connect to.
...
<ParamField path="search_path" type="string">
  ...
  **PostgreSQL only.**
  ...
  Without `search_path`, Badger DB MCP defaults to the `public` schema for all schema discovery operations.
