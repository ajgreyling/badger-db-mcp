...
npx badger-db-mcp --readonly --dsn "postgres://user:pass@localhost:5432/mydb"
```
</CodeGroup>
...
```bash Command Line (Deprecated)
# This flag is deprecated - use TOML configuration instead
npx badger-db-mcp --max-rows 1000 --dsn "..."
```
</CodeGroup>

- Only applied to SELECT statements, not INSERT/UPDATE/DELETE
- If your query already has a `LIMIT` or `TOP` clause, Badger DB MCP uses the smaller value
